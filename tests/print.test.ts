...
describe("Print this issue", () => {
  ...
  it("composes A4 and letter with the locked tokens and two families", async () => {
    ...
    const compositor = readFileSync("src/lib/print-compose.ts", "utf8");
    expect(compositor).toMatch(/243 \/ 255,\s*238 \/ 255,\s*228 \/ 255/);
    expect(compositor).toMatch(/28 \/ 255,\s*24 \/ 255,\s*20 \/ 255/);
    expect(compositor).toMatch(/61 \/ 255,\s*74 \/ 255,\s*58 \/ 255/);
    expect(compositor).not.toMatch(/250 \/ 255,\s*247 \/ 255,\s*241 \/ 255/);
    expect(compositor).not.toMatch(/74 \/ 255,\s*92 \/ 255,\s*86 \/ 255/);
