...
Then the More on this topic rail. Retrieval is a provider slot: one dedicated search API behind a function/config. The search API finds pages (raw). The app builds the query from topic + remaining entities + date — not keep-host / author-looking tokens (williamgomes from a Substack host). Empty topic falls back without the raw URL host. The app normalizes (Wikipedia hosts — any language or mobile `*.wikipedia.org` — are dropped), dedupes, ranks, drops pages with zero token overlap, and persists at most five `related_reporting`. Fewer results beat junk. Zero good hits after the filter is `ok`+0 (`nothingMoreOnTopic`), not fail. Wikipedia must never appear on More on this topic.
